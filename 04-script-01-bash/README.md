...
```
Не очень понятно, что подразумевается под фразой "пока один из узлов окажется недоступным". На доступность, согласно предыдущему заданию, мы проверяем сервис, а не сам хост. Если так - задание верное. Опять таки, на хосте может например на входе дропаться icmp, или по пути например, на fw, соответственно, корректно было бы проверить сервис, как я и выполнил.
...
Тогда в условии цикла While будет тождество в целях постоянного выполнения. Также, немного поменял циклы местами
